    ...
    email: {
      ...
      custom: {
        options: async (value, { req }) => {
          //dựa vào email để tìm user trong database
          ...
          req.user = user //lưu user vào request đê dùng ở controller
          return true
        }
      }
    },
  ...
  checkSchema(
    {
      email_verify_token: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            //nếu có email_verify_token ko gửi lên thì response lỗi 401
            if (!value) {
              throw new ErrorWithStatus({
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            try {
              ...
              //nếu có thì xem thử user này có forgot_password_token giống thằng client gữi lên hay không
